            ...
            <div class="rqa-hero">
                <div class="rqa-hero-content">
                    <div class="rqa-title-block">
                        <!-- <h4><?= h($title ?? 'RQA Approval (Observer)'); ?></h4> -->
                         <h4>RQA Approval</h4>
                        <p>Read-only view of the applicants recommended by the HRMPSB and awaiting approval. Use the filters to narrow the list &mdash; the list updates on its own as applicants are approved or declined.</p>
                    </div>
            ...
            <div class="card rqa-card rqa-results-card">
                <div class="card-body">
                    ...
                    <div id="rqa-loading" class="text-center text-muted rqa-loading-box" style="display:none;">
                        ...
                        <div class="mt-3 font-weight-bold">Loading recommended applicants&hellip;</div>
                    </div>
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    // change.select2 updates the widget WITHOUT firing our .on('change')
    // handlers, so rebuilding options never re-enters the render loop.

    function singleOptionsHtml(values, allLabel) {
        var html = '<option value="">' + allLabel + '</option>';
        values.forEach(function (v) { html += '<option value="' + escAttr(v) + '">' + escHtml(v) + '</option>'; });
        return html;
    }
    function multiOptionsHtml(values) {
        var html = '';
        values.forEach(function (v) { html += '<option value="' + escAttr(v) + '">' + escHtml(v) + '</option>'; });
        return html;
    }
    ...
    // still exists (used while polling); otherwise it resets to "All".
    function setSingle($el, values, allLabel, keep) {
        var prev = keep ? ($el.val() || '') : '';
        var valid = (prev === '') || values.some(function (v) { return v === prev; });
        $el.html(singleOptionsHtml(values, allLabel)).val(valid ? prev : '').trigger('change.select2');
    }

    // Rebuild a multi-select's options, optionally keeping still-valid choices.
    function setMulti($el, values, keep) {
        ...
        $el.html(multiOptionsHtml(values)).val(restored).trigger('change.select2');
    }
    ...
    function rowHtml(r, index) {
        ...
        html += '<td>' + (r.remarks ? escHtml(r.remarks) : '<span class="text-muted">&mdash;</span>') + '</td>';
        html += '</tr>';
        return html;
    }
    ...
    function selectedPositionRows() {
        var pos = $pos.val() || '';
        if (pos === '') return [];
        return allRows.filter(function (r) { return eqi(r.position, pos); });
    }
    ...
    function renderTable() {
        ...
        if (rows.length === 0) {
            $('#rqa-results').hide();
            if (allRows.length === 0) {
                setEmptyState('mdi-account-clock-outline', 'No applicants awaiting approval.', 'Recommended applicants from the HRMPSB will appear here automatically.');
            } else {
    ...
    // Rebuild every filter's option list. `keep` = true while polling so the
    // observer's current selections survive each refresh.
    function syncFilters(keep) {
        setSingle($pos, distinct(allRows.map(function (r) { return r.position; })), 'All Positions', keep);
        syncKindFilters(keep);
        setMulti($mun, distinct(allRows.map(function (r) { return r.municipality; })), keep);
        syncBarangays(keep);
    }
    ...
    function syncKindFilters(keep) {
        var kind = selectedPositionKind();
        var rows = selectedPositionRows();

        $('.rqa-kind-filter').hide();
        if (kind !== 'jhs') setMulti($jhsGroup, [], false);
        if (kind !== 'shs') { setMulti($strand, [], false); setMulti($major, [], false); }

        if (kind === 'jhs') {
            setMulti($jhsGroup, distinct(rows.map(function (r) { return r.specializationGroup || r.specialization; })), keep);
            $jhsGroupWrap.show();
        } else if (kind === 'shs') {
            setMulti($strand, distinct(rows.map(function (r) { return r.strand; })), keep);
            setMulti($major, distinct(rows.map(function (r) { return r.major; })), keep);
            $strandWrap.show();
            $majorWrap.show();
        }
    }
    ...
    $pos.on('change', function () { syncKindFilters(false); renderTable(); });
