    ...
    <body class="aip_generate" id="printTable">
        ...
        <div class="cert">
            ...
            <table class="rrr" id='myTable'>
                ...
                $applications = $this->Common->qualified_applicant_list_nt($jrow->jobID);
                $count=1;
                foreach($applications as $row){ 
                    ...
                    //     $rn = $a->record_no;
                    //     $rating = $this->Common->two_cond_row('hris_rating_none','appID',$row->appID,'record_no',$a->record_no);
                    //     // $ivy = $this->Common->three_cond_not_equal_count_row('hris_rating_none','record_no',$a->record_no,'fy',date('Y'),'total_points',0.00009);
                    // }else{
                    //     $a = $this->Common->one_cond_row('hris_staff','IDNumber',$row->empEmail);
                    //     $rn = $a->IDNumber;
                    //     $rating = $this->Common->two_cond_row('hris_rating_none','appID',$row->appID,'record_no',$a->IDNumber);
                    //     // $ivy = $this->Common->three_cond_not_equal_count_row('hris_rating_none','record_no',$a->IDNumber,'fy',date('Y'),'total_points',0.00009);
                    // }
                    // if(!empty($rating)){
                ...
                <tr>
                    ...
                    <td style="white-space: nowrap;">
                        <a style="text-decoration:none; color:#222;" target="_blank" href="<?= base_url(); ?>pages/<?= $row->st; ?>/<?= $row->id; ?>/<?= $row->jobID; ?>/<?= $row->pre_school; ?>/<?= $row->appID; ?>/<?= $row->code; ?>/" class="text-success tooltips" data-placement="top" data-toggle="tooltip" data-original-title="View Applicant Information"><?= $row->LastName.', '. $row->MiddleName.' '.$row->FirstName; ?></a> 
                        <a style="color:red" onclick="return confirm('Are You Sure?')" href="<?= base_url(); ?>Pages/application_delete_regular/<?= $row->appID; ?>">&#128465;</a> 
                    </td>
                    <td><?= $row->brgy; ?> <?= $row->resCity; ?></td>
                    <td class="text-center"><?= ($row->educ ?? '') == 0.00001 ? '' : ($row->educ ?? ''); ?></td>
                    <td class="text-center"><?= ($row->trainings ?? '') == 0.00001 ? '' : ($row->trainings ?? ''); ?></td>
                    <td class="text-center"><?= ($row->experience ?? '') == 0.00001 ? '' : ($row->experience ?? ''); ?></td>
                    <td class="text-center"><?= ($row->performance ?? '') == 0.00001 ? '' : ($row->performance ?? ''); ?></td>
                    <td class="text-center"><?= ($row->oa ?? '') == 0.00001 ? '' : ($row->oa ?? ''); ?></td>
                    <td class="text-center"><?= ($row->ae ?? '') == 0.00001 ? '' : ($row->ae ?? ''); ?></td>
                    <td class="text-center"><?= ($row->ald ?? '') == 0.00001 ? '' : ($row->ald ?? ''); ?></td>
                    <td style="background-color:#BFECFF;"><?= (!empty($rating) && !empty($user1) && $row->eval_id1 != 0) ? $user1->username : ''; ?></td>
                    <td class="text-center"><?= ($row->interview ?? '') == 0.00001 ? '' : ($row->interview ?? ''); ?></td>
                    <td class="text-center"><?= ($row->written ?? '') == 0.00001 ? '' : ($row->written ?? ''); ?></td>
                    <td class="text-center"><?= ($row->skills ?? '') == 0.00001 ? '' : ($row->skills ?? ''); ?></td>
