 ...
 <td class="wb">Division of <?php echo $settings->division; ?></td>
        ...
        <table class="data">
           ...
           <?php if (empty($is_excel_export)) flush(); $no = 1; ?>
                ...
                <?php
                    ...
                    // $resCity = trim(strtolower($ap->resCity ?? ''));
                    // $targetMun = trim(strtolower($this->input->get('mun')));
                    // if ($resCity !== $targetMun) continue;

                    // $brgy = isset($ap->resBarangay) ? strtoupper($ap->resBarangay) : (isset($ap->perBarangay) ? strtoupper($ap->perBarangay) : '');
                    // $contact = $ap->contactNo ?? '';
                    $oa = $this->Hiring_model->is_old_applicant($row->empEmail);
                ?>
                <tr>
                    <td><?= $no++; ?></td>
                    ...
                    <td style="font-size:8px"><?php if ($oa) {echo "SY " . (date('Y') - 1) . "-" . date('Y') . " Applicant";} ?></td>
        ...
        <table class="sign">
            ...
            <tr>
                ...
                <td><?php if($sign->m8p != ''){?><img class="isig" src="<?= base_url(); ?>assets/isig/<?= $sign->m8s; ?>.png" alt=""><span><?= $sign->m8n; ?></span><br /><?= $sign->m8p; ?><br />Member<?php } ?></td>
