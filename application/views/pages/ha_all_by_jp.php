        ...
        <table class="data" id="myTable" style="border:0">
            ...
            <tr>
                ...
                <td style="white-space: nowrap; text-align:left;">
                    <a style="color:#000; text-decoration:none" target="_blank"  href="<?= base_url(); ?>Pages/<?= $row->st; ?>/<?= $row->id; ?>">
                        <?= strtoupper($row->LastName.', '. $row->FirstName.' '.$row->MiddleName) ?>
                    </a>
                </td>
                <td style="text-align:left;"><?= strtoupper($row->brgy.', '. $row->resCity.' '.$row->province); ?></td>
                <td><?= $row->age; ?></td>
                <td><?= $row->sex; ?></td>
                <td><?= $row->ms; ?></td>
                <td><?= $row->religion ?? '' ?></td>
                <td><?php if(!empty($row)){ echo ($row->disability == 1) ? 'NO' : 'YES';} ?></td>
                <td><?= $row->ethnicity ?? '' ?></td>
                <td style="text-align:left;"><?= $row->email; ?></td>
                <td style="text-align:left;"><?= $row->cn; ?></td>
                <td><?= $row->bachelor; ?></td>
                <td>
