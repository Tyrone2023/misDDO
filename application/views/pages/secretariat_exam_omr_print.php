...
<body>
    ...
    <main class="paper">
        <h1><?= $h($exam->title); ?></h1>
        <div class="meta"><?= $h($exam->vacancy_title ?: $exam->job_title); ?> &middot; Exam <?= $h($exam->exam_code); ?></div>
        ...
        <div class="instructions"><strong>Instructions:</strong> Read each item carefully. Mark answers only on the separate OMR answer sheet using a dark pen or pencil. Completely fill each selected bubble and erase cleanly when changing an answer. On every answer-sheet page, shade your 10-digit application/examinee number, adding leading zeroes when necessary (example: application 123 becomes 0000000123).
