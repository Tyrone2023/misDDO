...
<div class="wrap">
    ...
    <h2 class="subtitle">SCHOOL WORK PLAN</h2>
    ...
    <table class="brigada">
        ...
        <?php foreach($kra as $row){
             $f = $this->Common->two_cond_row('brigada_kra_plan','kra_id',$row->id,'user_id',$this->session->username);
            ?>
        <tr>
            <td><?= $row->name; ?></td>
            <td><?= $f->activities; ?></td>
            <td><?= $f->timeline; ?></td>
            <td><?= $f->person_responsible; ?></td>
            <td><?= $f->materials_needed; ?></td>
            <td><?= $f->budget; ?></td>
        </tr>
        <?php } ?>
    </table>
